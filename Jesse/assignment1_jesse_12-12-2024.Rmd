...
We beginnen met een null model met alleen price_usd en intercept. Dit geeft het geschatte gemiddelde van price_usd; dat is 626.668 (intercept, estimate).
...
Op basis van de forward stepwise regression is de eerstvolgende predictor die toegevoegd wordt aa nhet model reviews_per_month.

Wanneer reviews_per_month wordt toegevoegd als predictor, dan is de gemiddelde geschatte price_usd 625.0372 (intercept estimate), wanneer reviews_per_month 0 is.

Op het moment dat reviews_per_month als predictor toegevoegd wordt, zien we dat als reviews_per_month met 1 unit stijgt, dat de geschatte gemiddelde price_usd met 1.1770 stijgt. Dit effect is niet significant.
...
AIC daalt met 1, wat lager is dus beter. 
BIC wordt 9 hoger, hoger is slechter. 
MSE wordt 3.8 lager, lager is beter.
De variantie van price_usd die wordt verklaard door het reviews_per_month is 3.422e-05. 

de deviance p value is > 0.05, dit suggereert dat het toevoegen van reviews_per_month het model niet significant verbetert ten op zichte van de fucking baseline.
...
Wanneer construction_year met 1 unit stijgt, daalt price_usd met 0.3066, gegeven dat reviews_per_month constant blijft. Negatief gecorreleerd.

Wanneer reviews_per_month met 1 unit stijgt, stijgt price_usd met 1.1834, gegeven dat construction_year constant blijft. (after controlling for the other variables in the model) Positief gecorreleerd.
...
We hebben handmatig cancellation_policy toegevoegd, om te laten dat we dit kunnen interpreteren, ook al weten we dat dit het model niet verbetert.

Het geschatte gemiddelde van price_usd voor flexible cancellation_policy is 629.2754 wanneer reviews_per_month en construction_year constant zijn/blijven. 

Het geschatte verschil in gemiddelde van price_usd voor moderate cancellation_policy vergeleken met flexible cancellation_policy is -2.1593, wanneer reviews_per_month en construction_year constant zijn/blijven. 

Het geschatte verschil in gemiddelde van price_usd voor strict cancellation_policy vergeleken met flexible cancellation_policy is -1.8438, wanneer reviews_per_month en construction_year constant zijn/blijven. 

Ze hebben dezelfde slope met een andere intercept, daarom voegen we een interactie effect toe. 
