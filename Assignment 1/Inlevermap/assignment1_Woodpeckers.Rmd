...
We are using a dataset found on [Kaggle](https://www.kaggle.com/datasets/arianazmoudeh/airbnbopendata) that contains information on Airbnb listings in New York. Airbnb is an American company that runs an online marketplace offering lodging, mainly vacation rental homestays, and tourism-related activities. We were interested in the nightly price of a stay at the listings in New York. This information is useful for determining the price of a new listing. The dataset has 26 variables that contain information about the listings. After assessing them, we chose to use 9 of the variables that could be relevant to the price. These variables are: cancellation_policy, room_type, review_rate_number, construction_year, service_fee, minimum_nights, number_of_reviews, reviews_per_month, and availability_365.

1. cancellation_policy: A variable with the values: flexible, moderate, and strict.
2. room_type: A variable with the values: Entire home/apt, Hotel room, Private room, Shared room. 
3. review_rate_number: Rounded average of the rating a review gave to a listing. The values range from 1 to 5.
4. construction_year: The year the property was built.
5. service_fee: The service fee in USD for the listing.
6. minimum_nights: Minimum number of nights needed for a booking of the listing.
7. number_of_reviews: Total number of reviews received for the listing.
8. reviews_per_month: The number of reviews the listing has over the lifetime of the listing per month.
9. availability_365: The availability of the listing x days in the future as determined by the calendar.
10. price: This variable is our target. We are trying to predict the price using the other variables. It describes how much a room costs per night.

## Research Question

“What variables are the best predictors for the price per night of Airbnb bookings in New York?”
...
The choice was made to only include the variables cancellation policy, room type, construction year, service fee, minimum nights, number of reviews, reviews per month, review rate number & availability 365 as possible predictors of price. Other variables in the dataset were either qualitative, such as complete written reviews or gpslocations of subareas in New York City, which we did **not** want to use for this analysis.
...
The decision was made to handle missing data with listwise deletion, which means excluding rows containing missing values from the analysis. This choice was made, because a complete dataset was preferred and it was possible, because the dataset contained over 60.000 observations.
...
Upon inspection of the data structure, it was recognized that the variables cancellation policy, room type & review rate number showed to be formatted as integers, when they should be interpreted as factors. Therefore these variables were refactored to the correct data type.
...
The variables price (USD) and service fee were of the string data type due to a dollar sign being present in the values. These variables were parsed to ensure the values were stored as integers instead of strings.
...
##### Table of value frequency for cancellation policy
...
##### Table of value frequency for room type
...
##### Table of value frequency for review rate number
...
The results show some unrealistic values for minimum nights. There are a few values below 1 and a few values above 365. The values bigger than 365 might indicate that people don't want their room to be rented, but do want to keep appearing in the Airbnb listing, which is a common strategy used by hosts ([Reddit, 2024](https://www.reddit.com/r/AirBnB/comments/1bg4oic/extremely_long_minimum_stay_policies_upwards_of/)). Therefore, we decided to remove values below 1 and above 365.
...
The results show some unrealistic values for availability_365. The description of this variable is 'how many days per year the Airbnb location is available'. Therefor we decided to remove negative values and values bigger than 365.
...
We do a final inspection of the data after all the cleaning is done. Listwise deletion of values for one variable can influence the distribution in the rest of the variables. 
...
A function was created to efficiently compare fit measures across models.
...
Since the goal is to identify predictors of Airbnb prices, forward stepwise selection was applied. Starting with an empty model, predictors were systematically added one at a time. The process continued until no further reduction in the AIC was possible, resulting in a model with the optimal AIC statistic. 

While AIC is used to select the optimal model, BIC and MSE were also evaluated to assess model fit quality, with lower values in each measure indicating a better fit.

### Model 0

The null model only contains the intercept, which is the estimated average of price_usd: 626.668
...
# Show null model summary
summary(model_null)
```

### Forward Stepwise Selection approach

Based on the forward stepwise selection approach that was used, the first predictor in the model that is suggested is `reviews_per_month`. The second predictor in the model that is suggested is `construction year`. After these predictors are added to the model, the model will **not** further improve according to this approach. However, when inspecting the small reductions in AIC the added predictors cause, the improvement is questionable, since the values show very minimal changes.
...
### Model 3: Price predicted by reviews per month and construction year. 

Based on the forward stepwise regression approach the second and last predictor that is added to the model is `construction_year`. The estimated average `price_usd` is 627.9384 when `reviews_per_month` and `construction_year` are held constant. When `construction_year` increases with one unit, the estimated average `price_usd` decreases with 0.3066, when controlling for `reviews_per_month`. This negative relation however is **not** significant ($p = 0.1265$). When `reviews_per_month` increases with one unit, the estimated average `price_usd` increases with 1.1834. This positive relation however is also **not** significant ($p = 0.0917$). The variance that is explained by both `reviews_per_month` and `construction_year` is still extremely small ($R^2 < .01$).

When comparing model 2 and 3 the results show no significant improvement of model 3 compared to model 2. The model deviance is 9040871201 with $p > .05$. Although the MSE decreases slightly, the AIC stays the same and the BIC increases.
...
One more model was manually checked to determine whether the factor `room_type` was a predictor of `price_usd`, after controlling for construction_year and reviews_per_month. Although we beforehand knew that this predictor would **not** improve the model, we wanted to add a categorical predictor with interaction effect to the analysis for the purpose of the assignment

The estimated average `price_usd` is 633.4169 for a `review_rate_number` of 1 when `construction_year` and `reviews_per_month` are held constant. The estimated difference in average `price_usd` decreases with 2.5368 for a `review_rate_number` of 2, compared to a `review_rate_number` of 1, for any `construction_year` or `reviews_per_month`. However, estimated difference in average is **not** significant. The only significant estimated difference in average `price_usd` at $\alpha = 0.05$ is between `review_rate_number` 1 compared to `review_rate_number` of 5. The estimated difference in average `price_usd` decreases with 10.0145 for a `review_rate_number` of 5, compared to a `review_rate_number` of 1, for any construction year or `reviews_per_month`.  

When adding an interaction to the model: `construction_year` and `review_rate_number`, the results show that there is a significant interaction effect between `review_rate_number2` and `construction year`, which can be interpreted as follows: the strength and direction of the relation between `construction_year` and `price_usd` is depends on the `review_rate_number`, but only `review_rate_number2`.
...
ggplot(data, aes(x = construction_year, 
                 y = price_usd, 
                 colour = review_rate_number)) +
  stat_smooth(method = 'gam', se = FALSE) +
  labs(
    title = "Interaction Effect: Construction Year, Price, and Review Rate Number",
    x = "Construction Year",
    y = "Price (USD)",
    colour = "Review Rate Number"
  ) +
  theme_bw()
```
...
```{r}
# Plot residuals
ggplot(mapping = aes(x = fitted(model_3),  
                     y = resid(model_3))) +
...
ggplot(mapping = aes(y = resid(model_3))) +
  geom_boxplot(fill = "lightblue", 
               color = "black", 
               outlier.color = "red", 
               outlier.shape = 20) +
  labs(
    title = "Boxplot of Residuals for Model 3",
    y = "Residuals",
    x = NULL  # No x-axis label for a single-variable boxplot
  ) +
  theme_bw()
```
...
After visual inspection of the histogram of residuals the conclusion is drawn that the assumption of normality is violated. A QQ-plot was requested to further investigate the suspected non-normality of errors.The QQ-plot clearly shows a S-shape, which is an indication that the normal distribution has heavy 'tails' or more extreme values. Because of the large sample size, formal testing; like the Shapiro test, could not be executed. These tests are very sensitive when sample sizes exceed 5000.

```{r}
# Histogram of residuals
ggplot(mapping = aes(x = resid(model_3))) +
  geom_histogram(
    binwidth = 10,
    fill = "lightblue", 
    color = "black"
  ...
  labs(
    title = "Histogram of Residuals for Model 3",
    x = "Residuals",
    y = "Frequency"
  ) +
  theme_bw()
...
ggplot(mapping = aes(sample = resid(model_3))) +
  stat_qq(color = "darkgrey") +
  stat_qq_line(color = "dodgerblue", linewidth = 1) +
  labs(
    title = "Q-Q Plot of Residuals for Model 3",
...
Multicollinearity was checked by evaulating VIF values. Upon evalution, it was concluded that the assumption of multicollinearity was **not** violated, since the VIF values did **not** exceed the threshold of 10. 
...
Because the plot is **not** interpretable due to the extreme sample size, the choice was made to only display studentized residuals larger than 3, which is a common threshold for big datasets, however, there are no student residuals bigger than 3 and also no student residuals bigger than 2, which is a common threshold for small datasets. Therefore the conclusion was drawn that there are no clear outliers to worry about

```{r}
# Create plot with studentized residuals
residuals_data <- data.frame(
  Index = seq_along(rstudent(model_3)),
  StudentizedResiduals = rstudent(model_3)
)

# Create plot with ggplot
ggplot(residuals_data, aes(x = Index, y = StudentizedResiduals)) +
  geom_point(color = "lightblue", shape = 20) +  # Points for residuals
  geom_hline(yintercept = 0, color = "gray", 
             linetype = "solid", linewidth = 1) +  # Horizontal line at 0
  labs(
    title = "Plot of Studentized Residuals",
    x = "Index",
...
```{r}
# Leverage values for the model
leverage_data <- data.frame(
  Index = seq_along(hatvalues(model_3)),
  Leverage = hatvalues(model_3)
)

# Create plot
ggplot(leverage_data, aes(x = Index, 
                          y = Leverage)) +
  geom_point(color = "lightblue", 
             shape = 20) +
  labs(
    title = "Leverage Values for Model 3",
    x = "Observation Index",
    y = "Leverage Value"
  ) +
  theme_bw()
...
```{r}
# Count the number of high-leverage points
sum(hatvalues(model_3) > 3 * mean(hatvalues(model_3)))
```

### Influential Cases

The DFBETAS was used to check for influential, and possible problematic observations per regression coefficient. The choice was made to plot influential cases for the intercept, reviews_per_month and construction_year. After visual inspection of the plots we saw that the intercept showed several influential cases. We extracted cases > |.02| based on visual inspection of the dfbetas plot  There are 11 influential cases. Based on visual inspection we also extracted infuential cases > |.05| for reviews per month. 6 influential cases were found. The influential cases found for reviewus per month overlap with the influential cases we found in price_usd*

We did **not** find influential cases for construction year.

```{r}
# Prepare data for dfbetas plots
dfbetas_data <- data.frame(
  Index = seq_len(nrow(data)),
  Intercept = dfbetas(model_3)[, 1],
  ReviewsPerMonth = dfbetas(model_3)[, 2],
  ConstructionYear = dfbetas(model_3)[, 3]
)

# Plot dfbetas for Intercept
ggplot(dfbetas_data, aes(x = Index, 
                         y = Intercept)) +
  geom_point(color = "lightblue", 
             shape = 20) +
  labs(
    title = "DFBETAS: Intercept",
    x = "Index",
    y = "DFBETAS"
  ) +
  theme_bw()

# Plot dfbetas for Reviews per Month
ggplot(dfbetas_data, aes(x = Index, 
                         y = ReviewsPerMonth)) +
  geom_point(color = "lightblue", 
             shape = 20) +
  labs(
    title = "DFBETAS: Reviews per Month",
    x = "Index",
    y = "DFBETAS"
  ) +
  theme_bw()

# Plot dfbetas for Construction Year
ggplot(dfbetas_data, aes(x = Index, 
                         y = ConstructionYear)) +
  geom_point(color = "lightblue", 
             shape = 20) +
  labs(
    title = "DFBETAS: Construction Year",
    x = "Index",
    y = "DFBETAS"
  ) +
  theme_bw()
...
```
```{r}
# Extract outstanding values for intercept
dfb_intercept <- dfbetas(model_3)[, 1]
out_obs_intercept <- dfb_intercept[
  dfb_intercept > 0.02 | dfb_intercept < -0.02
]
out_obs_intercept
```

```{r}
# Extract outstanding values for reviews_per_month
dfb_reviews <- dfbetas(model_3)[, 2]
out_obs_reviews <- dfb_reviews[
  dfb_reviews > 0.05 | dfb_reviews < -0.05
]
out_obs_reviews
```
...
```{r}
# Fit lm model with modified dataset
...
```{r}
# Normality check for residuals of Model 3.1 using Q-Q plot
ggplot(data = data.frame(Residuals = resid(model_3.1)), aes(sample = Residuals)) +
  stat_qq(color = "darkgrey") +
  stat_qq_line(color = "dodgerblue", 
               linewidth = 1) +
  labs(
    title = "Q-Q Plot of Residuals for Model 3.1",
    x = "Theoretical Quantiles",
    y = "Sample Quantiles"
  ) +
  theme_bw()
...
After removal of the 1907 points of large leverage and the influential cases there was a slight change in coëfficients noticable. The estimated average of `price_usd` was 627.8306 instead of 627.9384 in the original model when `reviews_per_month` and `construction_year` are held constant. When `construction_year` increases with one unit, the estimated average price (USD) decreases with 0.2853 instead of 0.3066 as in the original model, when controlled for `reviews_per_month`. This negative correlation is still **not** significant ($p = 0.1265$). When `reviews_per_month` increases with one unit, the estimated average `price_usd` increases with 1.1439 instead of 1.1834 as in the original model. This positive relation however is also **not** significant ($p = 0.0917$). The variance that is explained by both `reviews_per_month` and `construction_year` is still extremely small ($R^2 < .01$) and even smaller than the original model.

Although model meets the assumption of lineairity, homoscedasticity, independence of error, identified influential cases and points with large leverage were removed, this model can **not** be used to predict future `price_usd` for Airbnb's. This is due to the violation of assumption of normality, which is less of a big problem for bigger samples, but is always wanted for predicting intervals.

To answer our research question “What variables are the best predictors for the price per night of Airbnb bookings in New York?”, we can conclude that none of the variables have a significant predicting power. When considering the fit measures however, from all variables, `construction_year` and `reviews_per_month` have the most influence on the `price_usd` of a Airbnb listing.
