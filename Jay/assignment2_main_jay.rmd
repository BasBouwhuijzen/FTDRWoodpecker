...
summary(model_2)
```
Intercept of -8.11537, low probability of purchase, when age is 0 and gender 0 (female). The z and p values determine the intercept to be highly statistically significant. In the dataset, the minimum age is 18.
...
The coefficient for EstimatedSalary suggests that for every unit increase in salary, the log-odds of purchase increase by 0.00003644, while Gender and Age remain constant. This is statistically significant. This effect seems small, but do not forget that it applies to every single unit of currency. (stappen van duizend in de dataset) increment -> 1000 units of currency. Kaggle does little to describe the nature of the data. However, seeing the influence of salary, we can conclude the pruchased product is among luxury goods. EDIT
...
```
Model 3 is the best fit, with a much lower AIC value than the alternatives. The addition of Age and EstimatedSalary significantly improves the model fit, while Gender does not.

-Interaction effects
```{r}
# Logistic regression with multiple interactions
model_multiple_interactions <- glm(Purchased ~ Gender * Age + Age * EstimatedSalary + Gender * EstimatedSalary, data = data, family = binomial)
summary(model_multiple_interactions)
```
The coefficient for GenderMale:Age suggests that age has a slightly stronger effect for males compared to females, but this difference is not statistically significant (p = 0.404).

The coefficient for Age:EstimatedSalary negative and highly significant (p < 0.001):
This suggests that as Age increases, the positive effect of EstimatedSalary on purchasing weakens slightly.
For example, while a higher salary increases the likelihood of purchasing for younger individuals, its effect diminishes for older individuals.

The coefficient for GenderMale:EstimatedSalary is very small and not statistically significant (p = 0.894). This indicates that the relationship between EstimatedSalary and purchasing is consistent across genders.
